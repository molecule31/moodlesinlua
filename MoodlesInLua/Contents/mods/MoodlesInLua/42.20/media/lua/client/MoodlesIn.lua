    ...
    if player and player:getMoodles() then
        ...
        for moodleId = 0, numMoodles - 1 do
            ...
            if moodleLevel ~= 0 and moodleType ~= MoodleType.FOOD_EATEN or moodleLevel >= 3 then
                ...
                -- thanks UnCheat!
                local realBorderWidth = borderTexture and borderTexture:getWidth() or 128
                local realBorderHeight = borderTexture and borderTexture:getHeight() or 128
                local scaleFactor = moodleSize / 128

                -- FIXME may need to rely on user defined parameters but textures in the game are 128x128, so it's better to use 128 as a default
